...
func calories(distance float64) float64 {
    ...
    cal := (K1*Weight + (Speed*Speed/Height)*K2*Weight) * ((distance / Speed) / 60)
    return cal
}
...
func AcceptPackage(data string, storage []string) []string {
    ...
    // 7. Вычислить общее расстояние (в метрах)
     totalDistance := float64(totalSteps) * StepLength/1000
    // 8. Получить потраченные килокалории
    energy := calories (float64(totalDistance))
    // 9. Получить мотивирующий текст
    achiev := achievement(totalDistance)
...
Вы сожгли %.2f ккал.
%s`,t.Format("15:04:05"),totalSteps, totalDistance, energy, achiev ))
    showMessage(msg)
    return storage
}
